...
export default function IbmDDashboard() {
  ...
  const [liftingDateInput, setLiftingDateInput] = useState('');
  ...
  const handleWorkflowAction = async (action: 'approve' | 'reject') => {
    ...
    const isHazardous = selectedRequest.materialDetails.hazardousItems === 'Yes';
    const body: any = { remarks: remarksInput.trim() };

    if (action === 'approve' && !isHazardous) {
      if (!liftingDateInput.trim()) {
        Alert.alert('Validation Error', 'Lifting Date is required for normal items.');
        return;
      }
      body.liftingDate = liftingDateInput.trim();
    }

    setActionLoading(true);
    try {
      const endpoint = action === 'approve'
        ? `/api/requests/${selectedRequest._id}/ibmd-approve`
      ...
      if (response.data && response.data.success) {
        Alert.alert(
          'Action Completed',
          `Request ${selectedRequest.requestNo} has been processed successfully.`,
          [{ text: 'OK', onPress: () => {
      ...
      console.error('IBMD action error response:', error.response?.data || error.message);
    ...
    <SafeAreaView className="flex-1 bg-zinc-50" edges={['top', 'left', 'right']}>
      ...
      <View className="flex-row justify-between items-center px-4 py-3 bg-white border-b border-zinc-200">
        <View className="flex-row items-center">
          <Ionicons name="cube" size={24} color="#16a34a" style={{ marginRight: 8 }} />
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="flex-1 px-4 py-5">
        ...
        <View className="bg-[#16a34a] rounded-2xl p-6 mb-6 shadow-sm">
          <Text className="text-green-100 text-xs font-semibold uppercase tracking-widest">
            Welcome back, {user?.name ? user.name.split(' ')[0] : 'IBMD Officer'}
          </Text>
          ...
          <Text className="text-green-100 text-xs mt-2 leading-relaxed">
            Audit approved items. Normal items close directly with a Lifting Date; Hazardous items route to Sales.
          </Text>
        </View>
        ...
        <View className="flex-row bg-zinc-200/60 p-1.5 rounded-xl mb-6">
          ...
          >
            <Text className={`font-bold text-sm ${activeTab === 'pending' ? 'text-green-700' : 'text-zinc-600'}`}>
              Pending Audit ({pendingRequests.length})
            </Text>
          </TouchableOpacity>
        ...
        <View className="bg-zinc-100 border border-zinc-200 rounded-2xl p-4 mb-6">
          <Text className="text-zinc-800 text-base font-bold">
            {activeTab === 'pending' ? 'Audit Pipeline' : 'Action History'}
          </Text>
          <Text className="text-zinc-500 text-xs mt-1">
            {activeTab === 'pending'
              ? 'Below is the list of Gate Pass requests currently awaiting IBMD audit.'
          ...
          {activeTab === 'pending' ? (
            loadingRequests ? (
              ...
              <View className="bg-white border border-zinc-200 rounded-2xl p-8 items-center justify-center">
                ...
                <Text className="text-zinc-600 text-sm font-bold mt-4">Queue Cleared!</Text>
                <Text className="text-zinc-400 text-xs text-center mt-1">No requests are currently pending IBMD audit.</Text>
                <TouchableOpacity
                ...
                >
                  ...
                  <View className="mb-3">
                    ...
                    <Text className="text-zinc-700 text-sm font-semibold">
                      {req.materialDetails.itemType} - {req.materialDetails.itemCategory}
                      {req.materialDetails.hazardousItems === 'Yes' ? ' (HAZARD)' : ' (NORMAL)'}
                    </Text>
                  </View>
                  ...
                  <View className="border-t border-zinc-100 pt-3 flex-row justify-between items-center">
                    ...
                    <Text className="text-[#16a34a] text-xs font-bold flex-row items-center">
                      Audit request <Ionicons name="arrow-forward" size={12} color="#16a34a" />
                    </Text>
                  </View>
                </TouchableOpacity>
              ))
            ...
            loadingHistory ? (
              ...
              <View className="bg-white border border-zinc-200 rounded-2xl p-8 items-center justify-center">
                ...
                <Text className="text-zinc-400 text-xs text-center mt-1">You have not processed any audits yet.</Text>
              </View>
            ) : (
          ...
          {selectedRequest && (
            <ScrollView className="flex-1 p-5">
              ...
              <View className="space-y-4 mb-12">
                  ...
                  <View className="space-y-2">
                    ...
                    <View className="flex-row justify-between mb-2">
                      <View className="flex-1 mr-2">
                        ...
                        <Text className="text-zinc-700 text-sm font-bold">{selectedRequest.materialDetails.hazardousItems}</Text>
                      </View>
                      <View className="flex-1">
                        <Text className="text-zinc-400 text-xs font-medium">Alloy Type</Text>
                ...
                {/* Audit Action Panel */}
                {selectedRequest.status === 'pending_ibmd' && (
                  <View className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm mb-6">
                    ...
                    {/* Lifting Date for Normal Item */}
                    {selectedRequest.materialDetails.hazardousItems !== 'Yes' && (
                      <View className="mb-4">
                        <Text className="text-zinc-700 text-xs font-semibold mb-2 uppercase tracking-wider">Lifting Date *</Text>
                        <View className="bg-zinc-50 border border-zinc-300 rounded-xl px-4 py-3">
                          <TextInput
                            value={liftingDateInput}
                            onChangeText={setLiftingDateInput}
                            className="text-zinc-900 text-base"
                            placeholder="Enter Lifting Date (e.g. YYYY-MM-DD)"
