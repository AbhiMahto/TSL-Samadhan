...
export default function SalesDashboard() {
  ...
  const handleWorkflowAction = async (action: 'close' | 'reject') => {
    ...
    if (action === 'close') {
      if (!doNoInput.trim()) {
        Alert.alert('Validation Error', 'DO No (Delivery Order Number) is required.');
        return;
      }
      body.doNo = doNoInput.trim();
    }

    setActionLoading(true);
    try {
      const endpoint = action === 'close'
        ? `/api/requests/${selectedRequest._id}/sales-close`
      ...
      if (response.data && response.data.success) {
        Alert.alert(
          'Action Completed',
          `Request ${selectedRequest.requestNo} has been successfully closed.`,
          [{ text: 'OK', onPress: () => {
      ...
      console.error('Sales action error response:', error.response?.data || error.message);
      Alert.alert('Action Error', error.response?.data?.message || 'Failed to process action.');
    } finally {
    ...
    <SafeAreaView className="flex-1 bg-zinc-50" edges={['top', 'left', 'right']}>
      ...
      <View className="flex-row justify-between items-center px-4 py-3 bg-white border-b border-zinc-200">
        <View className="flex-row items-center">
          <Ionicons name="cart" size={24} color="#16a34a" style={{ marginRight: 8 }} />
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="flex-1 px-4 py-5">
        ...
        <View className="bg-[#16a34a] rounded-2xl p-6 mb-6 shadow-sm">
          <Text className="text-green-100 text-xs font-semibold uppercase tracking-widest">
            Welcome back, {user?.name ? user.name.split(' ')[0] : 'Sales Agent'}
          </Text>
          ...
          <Text className="text-green-100 text-xs mt-2 leading-relaxed">
            Close audited hazardous gate passes by registering Delivery Order (DO) numbers.
          </Text>
        </View>
        ...
        <View className="flex-row bg-zinc-200/60 p-1.5 rounded-xl mb-6">
          ...
          >
            <Text className={`font-bold text-sm ${activeTab === 'pending' ? 'text-green-700' : 'text-zinc-600'}`}>
              Pending Closure ({pendingRequests.length})
            </Text>
          </TouchableOpacity>
        ...
        <View className="bg-zinc-100 border border-zinc-200 rounded-2xl p-4 mb-6">
          <Text className="text-zinc-800 text-base font-bold">
            {activeTab === 'pending' ? 'Closure Desk' : 'Action History'}
          </Text>
          <Text className="text-zinc-500 text-xs mt-1">
            {activeTab === 'pending'
              ? 'Below is the list of Gate Pass requests currently awaiting DO registration and closure.'
          ...
          {activeTab === 'pending' ? (
            loadingRequests ? (
              ...
              <View className="bg-white border border-zinc-200 rounded-2xl p-8 items-center justify-center">
                ...
                <Text className="text-zinc-400 text-xs text-center mt-1">No requests are currently pending Sales closure.</Text>
                <TouchableOpacity
              ...
              pendingRequests.map((req) => (
                ...
                >
                  ...
                  <View className="mb-3">
                    <Text className="text-zinc-400 text-[10px] uppercase tracking-wider font-semibold">Material Description</Text>
                    <Text className="text-zinc-700 text-sm font-semibold">{req.materialDetails.itemDescription}</Text>
                  </View>
                  ...
                  <View className="border-t border-zinc-100 pt-3 flex-row justify-between items-center">
                    ...
                    <Text className="text-[#16a34a] text-xs font-bold flex-row items-center">
                      Close Request <Ionicons name="arrow-forward" size={12} color="#16a34a" />
                    </Text>
                  </View>
                </TouchableOpacity>
              ))
            ...
            loadingHistory ? (
              ...
              <View className="bg-white border border-zinc-200 rounded-2xl p-8 items-center justify-center">
                ...
                <Text className="text-zinc-400 text-xs text-center mt-1">You have not processed any closures yet.</Text>
              </View>
            ) : (
          ...
          {selectedRequest && (
            <ScrollView className="flex-1 p-5">
              ...
              <View className="space-y-4 mb-12">
                ...
                {/* Sales Closure Actions */}
                {selectedRequest.status === 'pending_sales' && (
                  <View className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm mb-6">
                    <Text className="text-zinc-900 font-extrabold text-sm mb-3 uppercase tracking-wider">Closure Action</Text>
                    
                    {/* DO No Input */}
                    <View className="mb-4">
                      <Text className="text-zinc-700 text-xs font-semibold mb-2 uppercase tracking-wider">DO No (Delivery Order Number) *</Text>
                      <View className="bg-zinc-50 border border-zinc-300 rounded-xl px-4 py-3">
                        <TextInput
                          value={doNoInput}
                          onChangeText={setDoNoInput}
                          className="text-zinc-900 text-base"
                          placeholder="Enter DO No (e.g. DO-500123)"
                    ...
                    <View className="flex-row space-x-3">
                      ...
                      <TouchableOpacity
                        onPress={() => handleWorkflowAction('close')}
                        disabled={actionLoading}
